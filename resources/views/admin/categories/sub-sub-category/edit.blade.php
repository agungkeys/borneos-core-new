...
<div class="app-main__inner">
   ...
   <div class="main-card mb-3 card">
      <div class="card-body">
            <form action="{{ route('admin.master-sub-category.update',$sub_sub_category->id ) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
               @method('PUT')
               @csrf
               <div class="form-group">
                   ...
                   <select class="form-control js-select2-custom" name="category" id="category" required>
                        @foreach ($sub_categories as $sub_category)
                            <option {{ $sub_category->parent_id == $sub_category->parent_id ? 'selected':''  }} value="{{ $sub_category->parent_id }}">{{ $sub_category->name }}</option>
                        @endforeach
                   </select>
               </div>
               <div class="form-group">
                  <label for="sub-category-name">Category Name</label>
                  <input type="text" id="sub-category-name" name="sub-category-name" value="{{ $sub_sub_category->name }}" class="form-control form-control-sm">
                ...
                <div class="form-group">
                  ...
                  <input type="text" id="sub-category-slug" name="sub-category-slug" value="{{ $sub_sub_category->slug }}" class="form-control form-control-sm">
