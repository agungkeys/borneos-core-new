    ...
    public function toTelegram($notifiable)
    {
        switch ($notifiable) {
            ...
            case $notifiable->status == 'otw' && $notifiable->orders->count() > 0:
                ...
                break;
            case $notifiable->status == 'cancel':
                $text = $this->textNotifyOrderCanceledToTelegram($notifiable);
                Telegram::sendMessage([
                    'chat_id' => Config::get('telegram.bots.mybot.chat_id'),
                    'parse_mode' => 'HTML',
                    'text' => $text
            ...
            case $notifiable->status == 'delivered':
                ...
                break;
            case $notifiable->status == 'done':
                $text = $this->textNotifyOrderDoneToTelegram($notifiable);
                Telegram::sendMessage([
                    'chat_id' => Config::get('telegram.bots.mybot.chat_id'),
                    'parse_mode' => 'HTML',
                    'text' => $text
