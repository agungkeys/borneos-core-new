...
trait Notify
{
    ...
    public function textNotificationOrderOnTheWaytoTelegram($data)
    {
        ...
        $textIntro = "\n\n<b>$orderStatus</b>\n$urlEditOrder\n\nHalo Kak, saya $customerName\ningin konfirmasi pesanan dari :\n\n$merchantName,\n$merchantAddress,\nTelp : $merchantTelp $line";
        $detail = $this->totalDistancePriceWithCustomerAddress($order);
        $courierName = $order->courier_id && $order->courier ? "".$order->courier->name : "-";
        $courierPhone = $order->courier_id && $order->courier && $order->courier->phone ? $this->replacePhoneFormatWA($order->courier->phone) : "-";
        $textCourier = "Kurir : \n$courierName\n$courierPhone";

        if($order->orders->count() == 1){
            $cart_1 = $this->listCart($order->orders[0]);
            return "".$textIntro .$cart_1 .$detail.$textCourier;
        } elseif($order->orders->count() == 2){
            $cart_1 = $this->listCart($order->orders[0]);
            $cart_2 = $this->listCart($order->orders[1]);   
            return "".$textIntro .$cart_1 ."\n".$cart_2 .$detail.$textCourier;
        }elseif($order->orders->count() == 3){
            $cart_1 = $this->listCart($order->orders[0]);
            $cart_2 = $this->listCart($order->orders[1]);
            $cart_3 = $this->listCart($order->orders[2]);
            return "".$textIntro .$cart_1 ."\n".$cart_2 ."\n".$cart_3 .$detail.$textCourier;
        }elseif($order->orders->count() == 4){
            $cart_1 = $this->listCart($order->orders[0]);
            $cart_2 = $this->listCart($order->orders[1]);
            $cart_3 = $this->listCart($order->orders[2]);
            $cart_4 = $this->listCart($order->orders[3]);
            return "".$textIntro .$cart_1 ."\n".$cart_2 ."\n".$cart_3 ."\n".$cart_4 .$detail.$textCourier;
        }elseif($order->orders->count() == 5){
            $cart_1 = $this->listCart($order->orders[0]);
            $cart_2 = $this->listCart($order->orders[1]);
            $cart_3 = $this->listCart($order->orders[2]);
